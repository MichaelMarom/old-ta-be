...
async function sendingSMS(req, res) {
    try {
        ...
        if (!instance?.data?.accessToken) throw new Error(`Failed to send Message with code ${instance.data.code} for accessToken`)
        const pathOfLogo = path.join(__dirname, "../templates/general/images/favicon.png")
        ...
        if (!response?.data?.sendResults?.length) throw new Error("Failed To Send message!. Please check text message and try again.")

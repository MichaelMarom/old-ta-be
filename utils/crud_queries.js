const insert = (tableName, values) => {
  ...
  const query = `INSERT INTO ${tableName} ${
    fieldsArray.length ? queryFieldsPart : null
  } OUTPUT inserted.* VALUES ${queryValuesPart}`;
  // console.log(query)
  return query;
};
...
const update = (
  ...
  const setClause = updateFieldsArray
    .map((field, index) => {
      const updatedValue =
  ...
  const whereClause = whereFieldsArray
    .map((field, index) => {
      const whereValue =
      ...
      return `${
        casting[field] ? `CAST(${field} AS ${fieldCasting})` : `${field}`
      } = ${whereValue}`;
    })
    .join(" AND ");
...
const parameterizedInsertQuery = (tableName, values) => {
  const fieldsArray = Object.keys(values).map(column => column==="end" ? `[${column}]`: column);
  const valuesArray = Object.keys(values).map((key) =>  values[key]);

  const valuePlaceholders = Object.keys(values).map((field) => `@${field}`).join(", ");

  const query = `
        INSERT INTO ${tableName} ${
    fieldsArray.length ? `(${fieldsArray.join(", ")})` : ""
  }
        OUTPUT inserted.*

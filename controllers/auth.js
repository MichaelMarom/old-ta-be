...
const verifyToken = async (req, res, next) => {
  if (
    ...
    req.originalUrl === "/admin/user/list"
  )
    next();
  ...
  if (!token || token === "undefined" || token === "null")
    return res
      .status(401)
      .json({
        reason: "token not attached!",
        message:
          "The security token has expired , please login again to continue protecting your account.",
      });
        ...
        if (err) {
          return res
            ...
            .json({
              reason: err.message,
              message:
                "The security token expired after 20 minutes of ideling, please login again.",
            });
        }
        req.user = decoded;
        next();
  ...
  marom_db(async (config) => {
    try {
      ...
      if (poolConnection) {
        const result = await poolConnection
          .request()
          .query(find("Users", { email: req.body.email }));
  ...
  marom_db(async (config) => {
    try {
      ...
      if (poolConnection) {
        const result = await poolConnection
          .request()
          .query(findByAnyIdColumn("Users", req.params));
        if (!result.recordset.length)
          throw new Error(`user with email = ${email} not found`);
        const hashedPassword = await bcrypt.hash(req.body.password, 10);
        const updateData = await poolConnection
          .request()
          .query(update("Users", { password: hashedPassword }, { email }));
